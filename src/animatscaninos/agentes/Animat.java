...
public class Animat implements Runnable {
	...
	private void Sensado() {
		...
		int i = 0;
		// Coordenadas X y Y y radios de distancia a los elementos mÃ¡s cercanos
		...
		for(i = 0; i < Mnd.iNumeroAlimentos; i++) {
			...
			// Determinan la distancia del plato de agua o comida mÃ¡s cercanos
		...
		for(i = 0; i < Mnd.iNumeroAnimats; i++) {
			if (Mnd.Perro[i] != this) { // Esta condicion evita que el Animat se sense a sÃ­ mismo
				...
				// Determinacion de la condicion de cercanÃ­a del Animat
